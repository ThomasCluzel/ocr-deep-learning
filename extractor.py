...
class Slicer:
	...
	def slicing(self,image, size=28):
		...
		#croping the bounding box in the original image
	...
	def space_invader(self, slicer2):
		...
		"""
		ret = ""
		print("{} {}".format(slicer2.iMin-self.iMax,self.iMax-self.iMin))
...
def saveChars(charTab,name,image):
	...
	Args:
		...
		name: name of the original image file
...
def char_detector(img_filename):
	...
	try:
		...
		for i in range(0,len(charTab)-1):
			img = charTab[i].slicing(image)
			character, _ = nn.guess([picture2vector(img)])
			s = s + character
			c = charTab[i].space_invader(charTab[i+1]);
			s = s+c
		saveChars(charTab, img_filename, image)
		# TODO: slice and recognize the chars nsert spaces and newlines
		# chars = "".join(resultat de la fonction précédente)
		print(s);
		image.close()
		img.close()
		#return chars
	except IOError :
		print("Incorrect filename: please give a picture to this function")
		raise IOError
...
if __name__ == "__main__":
	if(len(sys.argv) >= 2):
		...
		print("Gimme a picture in first param, pls")
